...
def handle(req):
    ...
    res = requests.post('http://' + gateway_hostname + ':8080/function/azure-text-analytics', 
                        json.dumps(dict),
                        headers={'Content-Type': 'application/json'})

    if res.status_code != 200:
        print("Error with azure text analytics, expected: %d, got: %d\n" % (200, res.status_code))
        sys.exit(1)

    # Read the positive_threshold from configuration
    positive_threshold = float(os.getenv("positive_threshold", "0.4"))

    polarity = res.json()['sentiment_score']
